...
import { OrbitControls, TransformControls, Grid, useCursor, Html, Edges, useGLTF } from '@react-three/drei';
...
interface ModelPart {
    ...
    // Box: [w, h, d]
    // Cyl: [rt, rb, h, seg]
    // Prism: [topSize, bottomSize, height]
    // Trapezoid: [width, height, depth, topScaleX, topScaleZ]
    args: number[]; 
    color: string;
    children: ModelPart[];
    visible: boolean;
...
const Trapezoid: React.FC<{ args: number[], color: string, isSelected: boolean }> = ({ args, color, isSelected }) => {
    ...
    // USE MEMO FOR STABLE GEOMETRY GENERATION TO FIX EDGES
...
const EditablePart: React.FC<{ 
    ...
    // Show gizmo only if this is the SINGLE selected item to avoid ambiguity
        ...
        <>
            ...
            {showGizmo && groupRef.current && (
                <TransformControls 
                    ...
                    onMouseUp={() => {
                        if (groupRef.current) {
                            onTransform(part.id, 'position', groupRef.current.position.toArray());
                            onTransform(part.id, 'rotation', [groupRef.current.rotation.x, groupRef.current.rotation.y, groupRef.current.rotation.z]);
                            onTransform(part.id, 'scale', groupRef.current.scale.toArray());
                        }
                    }}
                />
            )}
...
export const ModelBuilder: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    }, [undo, redo, selectedIds, model]); // Add dependencies for delete
    ...
    // Get the SINGLE selected part if only one is selected for property editing
    ...
    const handleSelect = (id: string, multi: boolean) => {
        if (multi) {
            setSelectedIds(prev => {
                ...
                // Ensure we always have at least empty set or keep behavior? 
                // If clicking same item, it toggles off.
                return newSet;
            });
        } else {
    ...
    const handleSidebarUpdate = (id: string, prop: keyof ModelPart, value: any) => {
        saveToHistory();
        setModel(prev => updatePart(prev, id, { [prop]: value }));
    }
    ...
    const handleGroupSelection = () => {
        ...
        // 1. Verify all selected items share the same parent
        ...
        // Get the actual part objects to move
        ...
        // 3. Reconstruct Parent Children List
                ...
                <Canvas camera={{ position: [3, 3, 3], fov: 50 }} shadows>
                    ...
                    <group position={[0, -1, 0]}>
                        <EditablePart 
                            ...
                            onTransform={(id, prop, val) => setModel(prev => updatePart(prev, id, { [prop]: val }))}
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                onChange={(e) => handleSidebarUpdate(activePart.id, 'name', e.target.value)}
                        ...
                        <Vector3Input label="POSITION" value={activePart.position} onChange={(v) => handleSidebarUpdate(activePart.id, 'position', v)} />
                        <Vector3Input label="ROTATION" value={activePart.rotation} onChange={(v) => handleSidebarUpdate(activePart.id, 'rotation', v)} />
                        <Vector3Input label="SCALE" value={activePart.scale} onChange={(v) => handleSidebarUpdate(activePart.id, 'scale', v)} />
                                ...
                                <div>
                                    ...
                                    <div className="flex space-x-2">
                                        <input 
                                            ...
                                            onChange={(e) => handleSidebarUpdate(activePart.id, 'color', e.target.value)}
